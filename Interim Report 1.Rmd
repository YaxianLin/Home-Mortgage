...
library(ggplot2)
library(dplyr)
action= Mortgage %>% group_by(action_taken_name) %>%
  summarise(count = n()) %>%
...
ggplot(action, aes(x =action_taken_name, y = p)) +
  ...
  coord_flip()
```
The first thing we did was to inspect the key variable action_taken_name. According to the result, more than half of loans are originated. We need to find which factors make the difference.

## Histogram plot for gender
```{r}
Mortgage$gender = "0"
Mortgage$gender[Mortgage$applicant_sex == 1] = "Male"
Mortgage$gender[Mortgage$applicant_sex == 2] = "Female"
...
Mortgage %>%
  ...
  xlab("income")
```
The left skewed density plot shows that most of the applicants' income concentrate on $8,000 to $10,000. And there are much less applicants with high income.
...
```{r}
library(gmodels)
CrossTable(Mortgage$applicant_ethnicity_name, Mortgage$lo)
```
