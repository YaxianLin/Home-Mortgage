...
```{r}
logit1 <- glm(la ~ loan_amount_000s+applicant_sex+applicant_race+applicant_income_000s, data = Mortgage_train1, family = "binomial")
summary(logit1)
...
Then, we used the predict1 model and test set to test the model. It turns out the accuracy is 69.05%.

```{r,eval=FALSE}
logit2 <-glm(la ~ loan_amount_000s+applicant_sex+applicant_race+applicant_income_000s+owner_occupancy, data = Mortgage_train1, family = "binomial")
summary(logit2)
...
logit3 <-glm(la ~ loan_amount_000s+applicant_sex+applicant_race+applicant_income_000s+owner_occupancy + population + property_type_1 + loan_purpose_1 , data = Mortgage_train1, family = "binomial")
summary(logit3)
...
【【【【【【【 could create a table here with variables in the model the the predicted accuracy

It turns out that the logit3 model has the highest predict accuracy, which is 70.73%.
# classification tree
...
tree1<-rpart(as.factor(la)~loan_amount_000s+applicant_sex+applicant_race+applicant_income_000s+owner_occupancy + population + property_type_1 + loan_purpose_1, data = Mortgage_train1)
prp(tree1)
tree_pred <-predict(tree1,Mortgage_test1)
confusionMatrix(data = tree_pred, Mortgage_test1$la)
```
